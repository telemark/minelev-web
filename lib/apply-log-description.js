module.exports = logs => {
  function logDescription (log) {
    ...
    if (log.lokalPlanMaal && Array.isArray(log.lokalPlanMaal) && log.lokalPlanMaal.length > 0) {
    ...
    if (/notat/.test(log.documentCategory)) {
      descriptions.push('Gå til MinElev - elevmappa for å lese notatet')
    }
