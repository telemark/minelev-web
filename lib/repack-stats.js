...
module.exports = data => {
  ...
  if (features.useNotes !== false) {
    if (data.totalNotat && data.totalNotat.total) {
      stats.push({name: 'Totalt antall notater til elevmappa', count: data.totalNotat.total || 0})
    } else {
      stats.push({name: 'Totalt antall notater til elevmappa', count: 0})
    }
  }
  ...
  if (features.useNotes !== false) {
    if (data.schoolsNotat && Array.isArray(data.schoolsNotat)) {
      if (data.schoolsNotat.length > 0) {
        stats.push({name: 'Notater til elevmappa fordelt pr skole', count: ''})
      }
      data.schoolsNotat.forEach(item => {
        if (item._id !== null) {
          stats.push({name: item._id, count: item.total})
        }
      })
