...
module.exports = options => {
  return new Promise(async (resolve, reject) => {
    ...
    logger('info', ['lib', 'search-logs', 'userId', options.userId, 'query', options.query, 'start'])
    try {
      const result = await axios.post(url, options.query)
      logger('info', ['lib', 'search-logs', 'userId', options.userId, 'query', options.query, 'success', result.data.length])
      resolve(result.data)
    } catch (error) {
